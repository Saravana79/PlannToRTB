		...
		public BiddingAgent
	{
		...
		*/
		...
		void setConfig()
		{
			...
			config.creatives.push_back(Creative::sample3201);

			config.exchangeFilter.include.push_back("adx");
			for (auto & c: config.creatives) {
				c.exchangeFilter.include.push_back("adx");
				c.providerConfig["adx"]["externalId"] = "PlannTo-Creative-2-S-%{meta.advertisementids}-" + RTBKIT::agent_ad_id;
				...
				c.providerConfig["adx"]["attribute"]  = "47 50 8";
			//	c.providerConfig["adx"]["attribute"]  = "7";
			...
			// whatever you feel it bests suits you.
					config.account = {"PlannToAccount_4" , "PC"};
			...
			// with our frequency cap augmentor example.
			{
				...
				// "urlMatcher"
				augConfig.filters.include.push_back("Advertisment-" + agent_ad_id + "-S-R");
...
int main(int argc, char** argv)
{
	...
	RTBKIT::FixedPriceBiddingAgent agent(serviceProxies, "HelloRTBKit-agent-" + RTBKIT::agent_ad_id + "-S-R");
	agent.init();
	agent.start();
